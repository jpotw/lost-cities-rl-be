...
def load_model(model_path="model_versions/model_final.pth", device=None):
    ...
    try:
        ...
        print(f"Warning: {model_path} not found. Starting fresh.")
        return None
    except Exception as e:
        print(f"Error loading model: {str(e)}")
        return None

def train(start_game=0, model_path="model_versions/"):
    env = LostCitiesEnv()
    
    # Load existing model if provided, else start fresh
    ...
    for game in range(start_game, NUM_GAMES):
        state = env.reset()
        done = False
...
if __name__ == "__main__":
    # Example: Resume from the last checkpoint (e.g., model_6000.pth)
    last_checkpoint = "model_versions/model_4000.pth"  # Replace with your last saved model
    start_game_number = 4000  # Match the checkpoint number
    if os.path.exists(last_checkpoint):
        train(start_game=start_game_number, model_path=last_checkpoint)
    else:
        print(f"Checkpoint {last_checkpoint} not found, starting fresh.")
        train()